...
from ui.shared import voice_ui
...
async def join_voice(guild: discord.Guild, channel: discord.VoiceChannel) -> bool:
    ...
    success = await voice_service.connect_channel(channel, self_deaf=True, self_mute=True)
    if success:
...
async def leave_voice(guild: discord.Guild) -> bool:
    ...
    success = await voice_service.disconnect_channel(guild)
    if success:
...
async def reconnect_voice(bot, guild: discord.Guild, channel_id: int):
    ...
    for i, wait in enumerate(backoff):
        ...
        try:
            ...
            logger.info(f"🔄 [Voice Chill Service] Intento de reconexión {i+1}/{len(backoff)} en {guild.name}...")
            ...
            logger.info(f"✅ [Voice Chill Service] Reconexión exitosa en {guild.name}.")
            return
        except Exception:
            logger.exception(f"❌ [Voice Chill Service] Fallo reconexión ({i+1})")
...
async def handle_voice_state_update(bot, member, before, after):
    ...
    if before.channel is not None and after.channel is not None and before.channel.id != after.channel.id:
        if member.guild.id in voice_targets:
            ...
            logger.info(f"🔄 [Voice Chill] Bot movido manualmente a {after.channel.name} en {member.guild.name}. Objetivo actualizado.")
        ...
        if target_channel_id and target_channel_id == before.channel.id:
            logger.warning(f"⚠️ [Voice Chill] Desconexión inesperada en {member.guild.name}. Iniciando reconexión...")
