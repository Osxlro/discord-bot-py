...
class WebServer:
    def __init__(self, app, host: str = "0.0.0.0", port: int = 5058):
        ...
        cert_path = current_dir / "config" / "cert.pem"
        key_path = current_dir / "config" / "key.pem"
        
        ssl_cert = str(cert_path) if cert_path.exists() else None
        ssl_key = str(key_path) if key_path.exists() else None
